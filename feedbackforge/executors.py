...
async def create_agent_with_ai_project(
    agent_name: str,
    instructions: str,
    ...
    """
    Helper function to create a ChatAgent using AIProjectClient pattern.
    ...
    agent = ChatAgent(
        ...
        name=agent_name,
        agent_id=agent_id or agent_version.id,
        instructions=instructions
    )
...
class InitialOrchestrator(Executor):
    ...
    async def _ensure_agent(self):
        """Lazy initialization of agent on first use."""
        if not self._agent_created:
            self.agent = await create_agent_with_ai_project(
